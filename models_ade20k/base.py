...
try:
    segm_options = dict(colors=loadmat(colors_path)['colors'],
                        classes=__import__('pandas').read_csv(classes_path),)
except ImportError:  # pandas absent; only needed for segmentation visualization
    segm_options = dict(colors=loadmat(colors_path)['colors'], classes=None)
