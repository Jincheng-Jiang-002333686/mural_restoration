...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512,
        ...
        # ABLATION (w/o MADF): standard strided convs replace mask-aware dynamic filters
        self.enc_conv.append(
            Conv2dLayer(in_channels=channel_schedule[0], out_channels=channel_schedule[0],
                        kernel_size=3, down=2, activation=activation)
        )
        self.enc_conv.append(
            Conv2dLayer(in_channels=channel_schedule[0], out_channels=channel_schedule[1],
                        kernel_size=3, down=2, activation=activation)
        )
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        # ABLATION (w/o MADF): plain strided convs, no mask propagation
        x = self.enc_conv[0](x)
