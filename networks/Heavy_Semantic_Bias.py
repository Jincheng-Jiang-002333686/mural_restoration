...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512,
        ...
        # CONSTRAINED CONDITIONALS ABLATION (s_img=360, s_latent=64, s_mask=16)
        self.to_style = FullyConnectedLayer(in_features=dim, out_features=dim * 2, activation=activation)
        self.ws_style = FullyConnectedLayer(in_features=w_dim, out_features=64, activation=activation)
        mask_style_channels = 16
        ...
        # The input features are now 360 + 64 + 16 = 440
        self.style_fusion = FullyConnectedLayer(in_features=dim * 2 + 64 + mask_style_channels, out_features=dim * 3)
        style_dim = dim * 3
