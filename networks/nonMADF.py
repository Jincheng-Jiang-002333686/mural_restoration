...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512, use_noise=False, demodulate=True,
        ...
        # Using Partial Convolutions for nonMADF
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        for i, block in enumerate(self.tran):  # 64 to 16
            if i < mid:
                ...
                # Setup ws processing for styles
        ...
        for i, block in enumerate(self.dec_conv):
            # Passing masks_in to the modified DecStyleBlock for Teacher-Forcing
            x, img = block(x, img, style, skips[len(self.dec_conv) - i - 1], masks_in, noise_mode=noise_mode)
