...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512, use_noise=False, demodulate=True,
        ...
        # Global style ONLY (Mask-Guided Style Encoder removed)
        down_conv = []
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        for i, block in enumerate(self.tran):
            if i < mid:
                ...
                # Style generation WITHOUT mask_style
                gs = self.to_style(self.down_conv(token2feature(x, x_size)).flatten(start_dim=1))

                # Just concatenate global style (dim*2) and w-latent style (dim) to get dim*3
                style = torch.cat([gs, ws_processed_dropped], dim=1)
        ...
        for i, block in enumerate(self.dec_conv):
            ...
            # Passing masks_in for Teacher-Forcing
