...
class DecStyleBlock(nn.Module):
    # __init__ remains completely unchanged from original MAT
    def __init__(self, res, in_channels, out_channels, activation, style_dim, use_noise, demodulate, img_channels):
        super().__init__()
        self.res = res
    ...
    # Standard skip connection, no mask logic here
...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512, use_noise=False, demodulate=True,
        ...
        # Use standard Conv for the very first layer before MADF
        self.conv_first = Conv2dLayer(in_channels=img_channels + 1, out_channels=channel_schedule[0], kernel_size=3,
                                      activation=activation)

        self.enc_conv = nn.ModuleList()
        # Bring back MADF Layers
        self.enc_conv.append(
            MADFLayer(in_channels=channel_schedule[0], out_channels=channel_schedule[0], kernel_size=3, down=2,
                      mask_channels=1, activation=activation))
        self.enc_conv.append(
            MADFLayer(in_channels=channel_schedule[0], out_channels=channel_schedule[1], kernel_size=3, down=2,
                      mask_channels=16, activation=activation))

        self.expand_final = Conv2dLayer(in_channels=channel_schedule[1], out_channels=dim, kernel_size=3,
                                        activation=activation)
        ...
        # Global style + Mask-Guided Style
        down_conv = []
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        for i, block in enumerate(self.tran):
            if i < mid:
                ...
                ws_processed_dropped = F.dropout(ws_processed, p=0.5, training=self.training)
                ...
                combined_styles = torch.cat([gs, ws_processed_dropped, mask_style], dim=1)
        ...
        for i, block in enumerate(self.dec_conv):
            ...
            x, img = block(x, img, style, current_skip, noise_mode=noise_mode)
