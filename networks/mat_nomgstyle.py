...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512,
        ...
        # ABLATION (w/o Mask-Guided Style): style fusion drops the mask-shape branch
        self.style_fusion = FullyConnectedLayer(in_features=dim * 2 + dim, out_features=dim * 3)
        style_dim = dim * 3
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        for i, block in enumerate(self.tran):
            if i < mid:
                ...
                # ABLATION (w/o Mask-Guided Style): no mask-shape style branch
                combined_styles = torch.cat([gs, ws_processed], dim=1)
