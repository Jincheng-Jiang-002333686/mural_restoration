...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512,
        ...
        # EQUAL CAPACITY ABLATION (s_img=180, s_latent=180, s_mask=180)
        self.to_style = FullyConnectedLayer(in_features=dim, out_features=dim, activation=activation)
        self.ws_style = FullyConnectedLayer(in_features=w_dim, out_features=dim, activation=activation)
        mask_style_channels = dim
        ...
        # The input features are now dim + dim + dim (180 + 180 + 180 = 540)
        self.style_fusion = FullyConnectedLayer(in_features=dim + dim + mask_style_channels, out_features=dim * 3)
        style_dim = dim * 3
